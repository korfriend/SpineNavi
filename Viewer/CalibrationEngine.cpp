...
bool CalibrationEngine::CheckCircleDetect(const cv::Mat& inputImg, const std::vector<cv::KeyPoint>& keyPoints)
{
...
void CalibrationEngine::CheckPositionSort(const cv::Mat& inputImg, const std::vector<cv::Point2f>& point2Ds, const std::vector<float>& radiis)
{
	cv::Mat img;
	...
	for (int i = 0; i < (int)point2Ds.size(); i++) {
		...
		// index 출력
		cv::putText(img, std::to_string(i), cv::Point(x - 20, y - 20), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255), 1);
	}
	cv::flip(img, img, 1);
	cv::imwrite(__gc->g_folder_trackingInfo + "test_circle_sort.png", img); // cv imshow()로 보이기.
}

bool CalibrationEngine::sortByDistance(const cv::Point2f& a, const cv::Point2f& b, const int m, const int n)
{
...
int CalibrationEngine::Get2DPostionsFromLegoPhantom(const cv::Mat& inputImg, std::vector<cv::Point2f>& points2Ds)
{
	cv::Mat imgGray;
	...
	if (inputImg.channels() == 3)
		...
		std::cout << "not supported image!" << std::endl;
		assert(0);
	}
	...
	for (int i = 0; i < row; i++) {
		...
		std::sort(circlePoints.begin(), circlePoints.end(),
			[&](const cv::Point3f& a, const cv::Point3f& b) {
				return sortByDistance(cv::Point2f(a.x, a.y), cv::Point2f(b.x, b.y), m, n); // sortByDistance에서 직선과 점의 거리 계산 후 정렬.
			});

		// 6개 or 8개 빼기			
		std::vector<cv::Point3f> rowPoints; // 뺀 점이 rowPoints에 들어갑니다.
	...
	// 포지션 잘 정렬되었는지 확인.
	CheckPositionSort(inputImg, points2Ds, circleRadiis);

	return (int)points2Ds.size();
}

int CalibrationEngine::Get2DPostionsFromFMarkersPhantom(const cv::Mat& inputImg, const int rows, const int cols, std::vector<cv::Point2f>& points2Ds)
{
	cv::Mat imgGray;
	int chs = inputImg.channels();
	if (inputImg.channels() == 3)
	...
	{
		std::cout << "there is a missing circle!!" << std::endl;
		return false;
	}
	...
	for (int i = 0; i < row; i++) {
		...
		// 5. x 방향으로 정렬.
		std::sort(rowPoints.begin(), rowPoints.end(), [](const cv::Point3f& a, const cv::Point3f& b) {
			return a.x < b.x;
			});
...
int CalibrationEngine::Get2DPostionsFromFMarkersPhantom2(const cv::Mat& inputImg, const int rows, const int cols, std::vector<cv::Point2f>& points2Ds)
{
	cv::Mat imgGray;
	int chs = inputImg.channels();
	if (inputImg.channels() == 3)
...
void CalibrationEngine::calc_normal_vector(const cv::Point3f& point1, const cv::Point3f& point2, const cv::Point3f& point3, cv::Point3f& vectorX, cv::Point3f& vectorY, cv::Point3f& normalVector)
{
...
cv::Point3f CalibrationEngine::fit_plane_to_points(const std::vector<cv::Point3f>& points)
{
	...
	std::vector<cv::Point3f> centered_points;
	for (const auto& point : points) {
...
int CalibrationEngine::Get3DPostionsFromLegoPhantom(const double marker_r, const cv::Point3f& marker1, const cv::Point3f& marker2, const cv::Point3f& marker3, const cv::Point3f& marker4, std::vector<cv::Point3f>& points3Ds)
{
...
int CalibrationEngine::Get3DPostionsFromLegoPhantom2(const float marker_r, const cv::Point3f& marker1, const cv::Point3f& marker2, const cv::Point3f& marker3, const cv::Point3f& marker4, std::vector<cv::Point3f>& points3Ds)
{
...
int CalibrationEngine::Get3DPostionsFromFMarkersPhantom(const glm::fvec3& marker0, const glm::fvec3& marker1, const glm::fvec3& marker2, const glm::fvec3& marker3, const track_info* trk, const int rows, const int cols, std::vector<cv::Point3f>& points3Ds)
{
	// note we are using meter metric
	using namespace std;
	using namespace glm;
...
void CalibrationEngine::InitializeTask(__GC* gcp)
{
	__gc = gcp;
...
int CalibrationEngine::RegisterCArmImage(const int sidScene, const std::string& carmScanParams, const std::string& scanName)
{
...
bool CalibrationEngine::CalibrationWithPhantom(glm::fmat4x4& matCArmRB2SourceCS, const cv::Mat& downloadedGrayImg, const track_info* trk, const bool useGlobal)
{
	...
	if (calib_mode == "LEGO") {
		Get2DPostionsFromLegoPhantom(source2detImg, points2d);
		if (points2d.size() != 60) {
		...
		if (det_mode == "SimpleBlobDetector")
			Get2DPostionsFromFMarkersPhantom(source2detImg, rows, cols, points2d);
		else if (det_mode == "HoughCircles")
			Get2DPostionsFromFMarkersPhantom2(source2detImg, rows, cols, points2d);
		else {
	...
	if (calib_mode == "LEGO") {
		Get3DPostionsFromLegoPhantom2(0.007f, posWsMarkers[0], posWsMarkers[1], posWsMarkers[2], posWsMarkers[3], points3d);
	}
	else if (calib_mode == "FMARKERS") {
		Get3DPostionsFromFMarkersPhantom(__cv3__ & posWsMarkers[0], __cv3__ & posWsMarkers[1], __cv3__ & posWsMarkers[2], __cv3__ & posWsMarkers[3],
